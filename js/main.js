(function($){
  ...
  function initApplicationForm() {
    ...
    $('.application-form').on('submit', function(event) {
      ...
      var requiredInputs = $(this).find('input[data-required="true"], textarea[data-required="true"]').toArray();
      ...
      if (isValidForm()) {
        ...
        var message = $(this).data('success-text') || 'Thanks for your application!<br>Your application has been sent. We will get back to you shortly!';
        var succesTemplate = '<div role="alert" class="alert alert-success alert-dismissible alert-colored text-center">'+ message +'</div>';
        $(this).find('[role="alert"]').fadeOut(300);
        $(succesTemplate).insertBefore($(this).find('button'));
        $(this).find('button')[0].disabled = true;
      }
    });

    $('.application-form input, .application-form textarea').on('keyup', function(event) {
      event.preventDefault();
      if ($(this).val()) {
        $(this).parent('.form-group').removeClass('has-error');
      }
